...
GAoptions.hostname = strtrim(hostname);
    ...
    %GAoptions.fitnessType = 'bandstructure';
    ...
    if strcmp(strtrim(hostname),'lotus-bud')
        GAoptions.dir = ['/home/daniel/GA_tetragonalRCD/','GA_',datestr(now,'mm-dd-yy_HH:MM'),'_N',int2str(GAoptions.popSize), '/']  %directory that all this goes into
    elseif strcmp(strtrim(hostname),'berzerk')
        GAoptions.dir = ['/home/danielbacon-brown/GA_8beam_tertragonalRCD/','GA_',datestr(now,'mm-dd-yy_HH:MM'),'_N',int2str(GAoptions.popSize), '/']  %directory that all this goes into
    elseif strcmp(strtrim(hostname),'Daniel-netbook')
        GAoptions.dir = ['C:/Users/daniel/GA_8beam_tetragonalRCD/','GA_',datestr(now,'mm-dd-yy_HH;MM'),'_N',int2str(GAoptions.popSize), '/']  %directory that all this goes into
    end
    ...
    %GAoptions.lattice = 'rectangular';
    GAoptions.lattice = 'rhombohedral';
    
    if strcmp(GAoptions.lattice, 'square') || strcmp(GAoptions.lattice, 'hexagonal')
        GAoptions.C_over_A = 1;    %Max C/A for air gap is 0.578 %for PDMS prism, max C/A = 1.396 %Will be overwritten if normal incidence
    elseif strcmp(GAoptions.lattice,'rectangular')
        %Relative values of A,B,C dimensions
        GAoptions.Arel = 1;
        GAoptions.Brel = 1.2;
        GAoptions.Crel = 1;
    elseif strcmp(GAoptions.lattice,'rhombohedral')
        GAoptions.Arel = 1;  %This is in terms of the canonical tetragonal lattice.  We actually work in a reduced rhombohedral lattice.
        GAoptions.Brel = 0.9;
        GAoptions.Crel = 1;
    end
    ...
    GAoptions.n_glassSubstrate = 1.52;   %Glass slide as substrate
    GAoptions.n_front = GAoptions.n_glassSubstrate; %GAoptions.n_PDMS;  %refractive index for incident light
    if strcmp(GAoptions.lattice,'rectangular')  %Assumes non-normal incidence
        %NOT CORRECT - NEED TO REDO MATH
        %Scale dimensions to get absolute values (um)
        s = GAoptions.laserWavelength / (2*GAoptions.n_PR) * sqrt(GAoptions.Arel^2+GAoptions.Brel^2+GAoptions.Crel^2);
        GAoptions.A = GAoptions.Arel * s; %um
        GAoptions.B = GAoptions.Brel * s;
        GAoptions.C = GAoptions.Crel * s;
    elseif strcmp(GAoptions.lattice,'rhombohedral') %Assumes normal incidence
        %This is for the canonical description of tetragonal lattice
        scaleFactor = GAoptions.laserWavelength/GAoptions.n_PR*sqrt( (1/GAoptions.Arel)^2 + (1/GAoptions.Brel)^2 + (1/GAoptions.Crel)^2 ); 
        GAoptions.A = GAoptions.Arel * scaleFactor; %um
        GAoptions.B = GAoptions.Brel * scaleFactor;
        GAoptions.C = GAoptions.Crel * scaleFactor;
        %This is for the rhombohedral lattice
        GAoptions.rhombohedralAngle = abs(2*atan(GAoptions.Arel/GAoptions.Brel));  %The angle that will result in the given amount of skewedness
        GAoptions.rhombohedralPeriod  = GAoptions.A/sin(1/2*GAoptions.rhombohedralAngle);
    end
...
%     S4interfaceOptions.layers(4) = Layer('Back','Glass', 0);
    ...
    if GAoptions.normalIncidence %Normally incident light
        ...
        elseif strcmp(GAoptions.lattice, 'rectangular')
            %NOT HANDLED
            error('rectangular lattice not handled for normal incidence')
            
        elseif strcmp(GAoptions.lattice, 'rhombohedral')
             GAoptions.periodX = GAoptions.rhombohedralPeriod;
             GAoptions.periodY = GAoptions.rhombohedralPeriod*sin(GAoptions.rhombohedralAngle);
        ...
        elseif strcmp(GAoptions.lattice, 'rectangular')
            GAoptions.periodX = GAoptions.A;
            GAoptions.periodY = GAoptions.B;
        elseif strcmp(GAoptions.lattice, 'rhombohedral')
            error('rhomobohedral lattice not supported for non-normal incidence')
        end
    ...
    elseif strcmp(GAoptions.lattice, 'rectangular')
        GAoptions.cells = floor( [25,25,25*GAoptions.repeatingUnits ])
    elseif strcmp(GAoptions.lattice, 'rhombohedral')
        %GAoptions.cells = floor(25*[1,1,GAoptions.Crel/GAoptions.Arel*GAoptions.repeatingUnits]);
        GAoptions.cells = floor(25*[1,1,1]);
    end
    if strcmp(GAoptions.lattice, 'square')
    ...
    elseif strcmp(GAoptions.lattice, 'rhombohedral')
        GAoptions.dimensions = [GAoptions.periodX, GAoptions.periodY, GAoptions.C];
        GAoptions.cellsCart = floor(20*[1,GAoptions.periodY/GAoptions.periodX,GAoptions.C/GAoptions.A*GAoptions.repeatingUnits]) %Cells for when converting to cartesian coordinates (plotting and Lumerical)
    end
    ...
    elseif strcmp(GAoptions.lattice, 'hexagonal')
        ...
        GAoptions.w = [0,0,GAoptions.period*GAoptions.C_over_A];
    elseif strcmp(GAoptions.lattice, 'rectangular')
        GAoptions.u = [GAoptions.A,0,0];
        GAoptions.v = [0,GAoptions.B,0];
        GAoptions.w = [0,0,GAoptions.C];
    elseif strcmp(GAoptions.lattice, 'rhombohedral')
        %Doing based on rhombohedral lattice
        GAoptions.u = [GAoptions.rhombohedralPeriod,0,0];
        GAoptions.v = [GAoptions.rhombohedralPeriod*cos(GAoptions.rhombohedralAngle),GAoptions.rhombohedralPeriod*sin(GAoptions.rhombohedralAngle),0];
        GAoptions.w = [0,0,GAoptions.C];
    end
    ...
    if strcmp(GAoptions.lattice, 'square') || strcmp(GAoptions.lattice, 'hexagonal')
        ...
        incidentLightOptions.periodX = GAoptions.periodX;
        incidentLightOptions.periodY = GAoptions.periodY;
    end
...
%For  GratingGrid
    ...
    elseif strcmp(GAoptions.lattice,'rectangular')
        gratingOptions.periodX = GAoptions.A;
        gratingOptions.periodY = GAoptions.B;   
    elseif strcmp(GAoptions.lattice, 'rhombohedral')
        gratingOptions.period = GAoptions.rhombohedralPeriod;
        gratingOptions.rhombohedralAngle = GAoptions.rhombohedralAngle;
    end
...
%     %%%%% Target Structure %%%%%
    ...
    targetStructureOptions.structureType = 'RCD_rhombus';
...
%     targetStructureOptions.structureType = 'helix';
    ...
    if strcmp(targetStructureOptions.structureType, 'doublehelix') || strcmp(targetStructureOptions.structureType, 'helix')
    ...
    elseif strcmp(targetStructureOptions.structureType, 'RCD_rhombus')
        targetStructureOptions.r = GAoptions.cells(1)/4;
    end
    %Generate helix patterns
    ...
    elseif strcmp(targetStructureOptions.structureType, 'RCD_rhombus')
        GAoptions.targetStructure = drawRCD_rhombus(GAoptions.cells,targetStructureOptions.r);
    end
...
%     GAoptions.edgeExclusionStructure = edgeExclusionStructure;
    ...
    fillOptions.fillMax = 0.7;
    fillOptions.fillMin = 0.3;
    ...
    %%%%% MPB SETTINGS %%%%%
    MPBoptions.hostname = GAoptions.hostname;
    MPBoptions.n_fill = GAoptions.n_PR;
    MPBoptions.n_void = 1;
    GAoptions.MPBoptions = MPBoptions;
    GAoptions.MPBinterface = MPBinterface(MPBoptions);
