...
%     S4interfaceOptions.layers(4) = Layer('Back','Glass', 0);
    ...
    elseif strcmp(GAoptions.lattice, 'hexagonal')
        GAoptions.dimensions = [GAoptions.period*sqrt(3)/2, GAoptions.period, GAoptions.period*GAoptions.C_over_A]; %dimensions of unit cell (cartesian)
...
...        + chromNmaterial ...
    ...
    + 8*4*3 ... %Es, Ep, phase (elliptical polarization)
    + 8 ... %incidentAngle
    ;
...
fitness = fitnessFunction_4beamExternal(GAoptions,chromosome) %Should also work for reflections
...
%%%%% Declaration of Fitness Function %%%%%
    function fitness = gfit(chromosome)  %This is done so that it can pass GAoptions to the fitness function.
        %tic
        fitness = fitnessFunction_4beamExternal(GAoptions,chromosome);
        %toc 
    end
