function gyroidStruct3 = generateDoubleGyroid_4beam(options)
%Cells is x,y,z, z-hat points in [1 1 1] direction of structure

%Generate gyroid structure:
%options.cells = [25,25,25];
options.cells = [floor(100*sqrt(3)/2),100,floor(100/2*1.7322/1.4142)];
...
linU = linspace(0,1.4142*2*pi*sqrt(3)/2,options.cells(1)); 
linV = linspace(0,1.4142*2*pi,options.cells(2)); 
linW = linspace(0,1.7322*pi,options.cells(3));
...
%     zlim([0 3])
% 
% %MAKE UNIT CELL PATTERN -parallelogramic prism
...
gyroidStruct3 = gyroidStruct1; %TESTING: IF we can make a single gyroid at 50% fill, we can make a skin, etch SU8, deposit conformally onto skin, then do a material inversion, yielding a double gyroid

disp(['actual fill: ', num2str(sum(sum(sum(gyroidStruct3)))/numel(gyroidStruct3))])
...
axis equal
lighting gouraud
xlim([1 options.cells(2)+2])
ylim([1 options.cells(1)+2])
zlim([1 options.cells(3)+2])
box on
%set(gca,'XTick',[],'YTick',[],'ZTick',[]);
