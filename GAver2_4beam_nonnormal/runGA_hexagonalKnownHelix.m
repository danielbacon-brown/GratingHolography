function runGA_hexagonalKnownHelix
...
GAoptions.hostname = strtrim(hostname);
    ...
    elseif strcmp(strtrim(hostname),'Daniel-netbook')
        GAoptions.dir = ['C:/Users/daniel/GA_8beamReflect_helix_data/','GA_ver2_',datestr(now,'mm-dd-yy_HH_MM'),'_N',int2str(GAoptions.popSize), '/']  %directory that all this goes into
    end
    ...
    GAoptions.C_over_A = 2;    %Max C/A for air gap is 0.578 %for PDMS prism, max C/A = 1.396 %Will be overwritten if normal incidence
...
%     S4interfaceOptions.layers(4) = Layer('Back','Glass', 0);
    ...
    %BEcause just helix pattern:
    GAoptions.period = 0.5334; %um
    GAoptions.C_over_A = 2;
...
%     end
    ...
    GAoptions.gratingType = 'GratingGrid';
    %GAoptions.gratingType = 'AddCircles';
    
    if strcmp(GAoptions.gratingType,'GratingGrid')
        %For  GratingGrid
        gratingOptions.NblockX = 4;
        gratingOptions.NblockY = 2;
...
%         gratingOptions.chromNSU8thickness = 8;
        ...
        gratingOptions.spacingMin = 0;  %Minimum block width not necessary for this optimization
        GAoptions.gratingOptions = gratingOptions;
        if strcmp(GAoptions.lattice, 'square')
            GAoptions.gratingFunction = GratingGridSquare(gratingOptions);
        elseif strcmp(GAoptions.lattice, 'hexagonal')
            GAoptions.gratingFunction = GratingGridHexagonalKnownHelix(gratingOptions);
        end
        disp(GAoptions.gratingFunction)
    elseif strcmp(GAoptions.gratingType, 'AddCircles')
        gratingOptions.NblockX = 40;
        gratingOptions.NblockY = 40;
        gratingOptions.Ncircles = 3;
        gratingOptions.circleRad = 0.05; %um
        gratingOptions.chromNxpos = 8;
        gratingOptions.chromNypos = 8;
        gratingOptions.n_filled = GAoptions.n_PR; %1.58;  %refractive index of grating material
        gratingOptions.n_void = GAoptions.n_gratingVoid;  %refractive index of void space of grating
        gratingOptions.periodicity = GAoptions.period;
        GAoptions.gratingOptions = gratingOptions;
        if strcmp(GAoptions.lattice, 'square')
            GAoptions.gratingFunction = GratingCirclesSquare(gratingOptions);
        elseif strcmp(GAoptions.lattice, 'hexagonal')
            GAoptions.gratingFunction = GratingCirclesHexagonal(gratingOptions);
        end
    ...
    targetStructureOptions.radl = GAoptions.period/7; %radius of the wire (um)
