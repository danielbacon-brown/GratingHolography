    ...
    methods
        ...
        function writeLumericalRunFileSquare( options, structureSU8)
            ...
            if options.fdtd.addCubesDirectly
                ...
                addCubesStr = [addCubesStr, sprintf('addrect; set("name","base"); set("x span",%e); set("y span",%e); set("z span",%e); \n', ...
                    dx, dy, dz) ];
                
                switch L.metal
                    case 'gold' 
                        sprintf('set("material","Au (Gold) - CRC"); \n');
                    case 'silver'
                        sprintf('set("material","Ag (Silver) - CRC"); \n');
                end
                
                addCubesStr = [addCubesStr, sprintf('set("x",%e); set("y",%e); set("z",%e); \n', -options.dimensions(1)/2*1e-6 - dx/2, -options.dimensions(2)/2*1e-6 - dy/2, -structureThickness/2*1e-6 - dz/2)];
                for i = 1:options.cells(1)
                    for j = 1:options.cells(2)
                        for k = 1:options.cells(3)*options.fdtd.repeatUnits
                            if ~structureSU8(i,j,k)
                                addCubesStr = [addCubesStr, sprintf('select("base"); copy(%e,%e,%e); set("name","%s"); \n',i*dx,j*dy,k*dz, 'C')];
                            end
                        end
                    end
                end
                addCubesStr = [addCubesStr, sprintf('select("base"); delete; \n')];
        ...
        function setXYstr = setXYdimensionStr()
            setXYstr = [ ...
                sprintf('set("x",%e); \n', 0) ...
                , sprintf('set("y",%e); \n', 0) ...
        ...
        function setWLstr = setWavelengthLimStr()
            setWLstr = [ ...
